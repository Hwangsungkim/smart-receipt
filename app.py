...
def calculate_true_unit_price(data):
    ...
    # [인간 친화적 UI 교정] 0번 멸종, 1번부터 인덱스 시작!
    if not df.empty:
...
def analyze_receipt(image, model_name):
    ...
    # [V11 AI 뇌 개조] 숫자는 절대 포기하지 않는 집념의 역산 지능 탑재
    prompt = """
    당신은 영수증, B2B 거래명세서, 간이 수기(손글씨) 영수증을 모두 판독하는 최고급 회계 AI입니다. 
    반드시 아래 JSON 객체 형식으로만 응답하세요. 마크다운 제외.
    
    [엄격한 예외 통제 지침 - V11 (본질 집중 모드)]
    1. **이름은 포기해도 숫자는 포기하지 마라**: 상품명을 도저히 못 읽겠으면 "[수기/판독불가]"로 적으세요. 하지만 **수량과 금액(숫자)은 절대로 포기하지 말고 반드시 찾아내세요.**
    2. **수기 특수기호 완벽 필터링**: 손글씨 영수증에서 금액이나 수량 뒤에 붙은 펜 자국(`-`, `=`, `ㄴ`, `ㄷ`, `F` 등)은 절대 문자로 읽지 말고 순수 아라비아 숫자만 발라내세요. (예: 10,- -> 10000, 12,- -> 12000, 70,- -> 70000)
    3. **역산(Reverse Engineering) 검증 필수**: 당신이 추출한 품목별 '원래가격'들을 모두 합쳤을 때, 영수증 맨 아래에 적힌 '총합계(예: 104000)'와 정확히 일치해야 합니다. 숫자가 헷갈리면 역산해서 퍼즐을 맞추세요!
