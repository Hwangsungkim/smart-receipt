...
def get_best_available_model():
    # [V13.1 긴급 패치] 하드코딩 제거! 속도가 빠른 Flash 모델을 1순위로 '안전하게' 탐색
    available_models = []
    for m in genai.list_models():
        if 'generateContent' in m.supported_generation_methods:
            available_models.append(m.name)
            
    priorities = ['models/gemini-1.5-flash-latest', 'models/gemini-1.5-flash', 'models/gemini-1.5-pro-latest', 'models/gemini-1.5-pro']
...
def analyze_receipt(image, model_name):
    ...
    [엄격한 예외 통제 지침 - V13.1 (도매시장 암호 해독 및 본질 집중)]
